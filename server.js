...
// Page Routes
app.get(["/admin", "/admin-login", "/admin-dashboard", "/operator-console", "/admin.html"], (req, res) => {
...
app.post("/api/admin/login", async (req, res) => {
    ...
    const adminUser = process.env.ADMIN_USERNAME || "somu";
    ...
    try {
        const match = (username === "somu" && password === "somu") || (adminHash && await bcrypt.compare(password, adminHash));
        if (!match) {
            recordFailedAttempt(ip);
            await AuditLog.create({
                email: username,
